...
# Simple autoencoder
To train an autoencoder we can use the [h2o package](https://cran.r-project.org/web/packages/h2o/index.html):
...
tr_te <- tr %>%
  bind_cols(tr_ae)  %>% 
  bind_rows(bind_cols(te, te_ae)) %>% 
  mutate(year = year(date) %>% factor(),
         wday = wday(date) %>% factor(),
         hour = hour(as_datetime(visitStartTime)),
         isMobile = ifelse(isMobile, 1L, 0L),
  ...
  mutate(pageviews_mean_vn = grp_mean(pageviews, visitNumber),
         ...
         pageviews_mean_metro = grp_mean(pageviews, referralPath)) %T>% 
  glimpse()
...
```
As it was stated earlier, pageviews and hits play important role in the XGB model. 
The model score on LB is 1.45.
...
pred_avg <- log1p(expm1(pred_glm) + expm1(pred_nn) + expm1(pred_xgb))
sub <- "avg_gs.csv"
submit(pred_xgb)
```
